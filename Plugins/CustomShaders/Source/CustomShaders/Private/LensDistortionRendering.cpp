...
/**
 * Internal intermediary structure derived from FCustomShadersCameraModel by the game thread
 * to hand to the render thread.
 */
struct FCompiledCameraModel
{
    /** Orignal camera model that has generated this compiled model. */
    FCustomShadersCameraModel OriginalCameraModel;
...
IMPLEMENT_SHADER_TYPE(, FLensDistortionUVGenerationVS, TEXT("/Plugin/CustomShaders/Private/MyShader.usf"), TEXT("MainVS"), SF_Vertex)
IMPLEMENT_SHADER_TYPE(, FLensDistortionUVGenerationPS, TEXT("/Plugin/CustomShaders/Private/MyShader.usf"), TEXT("MainPS"), SF_Pixel)
...
FVector2D FCustomShadersCameraModel::UndistortNormalizedViewPosition(FVector2D EngineV) const
{
...
float FCustomShadersCameraModel::GetUndistortOverscanFactor(
    float DistortedHorizontalFOV, float DistortedAspectRatio) const
{
    ...
    if (*this == FCustomShadersCameraModel())
    {
        return 1.0f;
    }
...
void FCustomShadersCameraModel::DrawUVDisplacementToRenderTarget(
    UWorld* World,

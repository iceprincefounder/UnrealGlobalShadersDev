...
class UCustomShadersBlueprintLibrary : public UBlueprintFunctionLibrary
{
	GENERATED_UCLASS_BODY()


	/** Returns the overscan factor required for the undistort rendering to avoid unrendered distorted pixels. */
	UFUNCTION(BlueprintPure, Category = "Lens Distortion X")
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Lens Distortion X", meta = (WorldContext = "WorldContextObject"))
	...
	UFUNCTION(BlueprintPure, meta=(DisplayName = "Equal (LensDistortionCameraModel)", CompactNodeTitle = "==", Keywords = "== equal"), Category = "Lens Distortion X")
	static bool EqualEqual_CompareLensDistortionModels(
		const FCustomShadersCameraModel& A,
		const FCustomShadersCameraModel& B)
	{
		return A == B;
	}

	/* Returns true if A is not equal to B (A != B) */
	UFUNCTION(BlueprintPure, meta = (DisplayName = "NotEqual (LensDistortionCameraModel)", CompactNodeTitle = "!=", Keywords = "!= not equal"), Category = "Lens Distortion X")
